...
class NT_Xent(nn.Module):
    ...
    def forward(self, z_i, z_j):
        ...
        mask = self.mask_correlated_samples(batch_size, self.world_size)    # only negative sample (different samples in batch)
        ...
        sim_i_j = torch.diag(sim, batch_size * self.world_size)     # the diagonal between z_i*z_j (B) -> positive
        sim_j_i = torch.diag(sim, -batch_size * self.world_size)    # the diagonal between z_j*z_i (B) -> positive
        ...
        labels = torch.zeros(N).to(positive_samples.device).long()          # 2B x 2B
