...
class CLSA(nn.Module):
    ...
    def __init__(self, backbone, pick_weak, sym, pick_strong, dim=128, K=65536, m=0.999, T=0.2, t=0.2, mlp=True):
        """
        ...
        self.weak_pick = pick_weak
        self.strong_pick = pick_strong
    ...
    def forward(self, im_q_list, im_k, im_strong_list, distributed=False):
        """
        ...
        if self.sym:
            ...
            with torch.no_grad():  # no gradient to keys
                ...
                if distributed:     im_k, idx_unshuffle = self._batch_shuffle_ddp(im_k)

                k = self.encoder_k(im_k)  # keys: NxC
                k = nn.functional.normalize(k, dim=1)
                # undo shuffle
                if distributed:     k = self._batch_unshuffle_ddp(k, idx_unshuffle)
                k = k.detach()
                if distributed:     k = concat_all_gather(k)

                k2 = self.encoder_k(im_q_list[0])  # keys: NxC
                k2 = nn.functional.normalize(k2, dim=1)
                # undo shuffle
                if distributed:     k2 = self._batch_unshuffle_ddp(k2, idx_unshuffle)
                k2 = k2.detach()
                if distributed:     k2 = concat_all_gather(k2)
            logits0_list = []
            labels0_list = []
            logits1_list = []
            labels1_list = []
            ...
            for choose_idx in range(len(q_list) - 1):
                ...
                labels = torch.arange(batch_size, 2 * batch_size, dtype=torch.long).cuda()
